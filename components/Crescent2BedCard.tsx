...
const IMAGES = Array.from({ length: 14 }, (_, i) => `/stays/crescent2/crescent2-${i + 1}.jpg`);

export default function Crescent2BedCard() {
    ...
    <>
      ...
      <div className="group cursor-pointer">
        <div className="relative aspect-[4/5] mb-6 overflow-hidden" onClick={() => openLightbox(current)}>
          {IMAGES.map((src, i) => (
            <img key={i} src={src} alt={`OPV Crescent 2-Bed view ${i + 1}`}
          ...
          <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-1 z-10 flex-wrap px-4">
            {IMAGES.map((_, i) => (
        ...
        <div className="flex justify-between items-start mb-2">
          <h4 className="font-headline-sm text-on-surface">OPV Crescent — 2 Bed</h4>
        ...
        <p className="text-on-surface-variant font-body-md line-clamp-1 mb-4">A spacious two-bedroom apartment with contemporary interiors and city views.</p>
        <div className="flex justify-between items-center pt-4 border-t border-outline-variant/20">
          <span className="text-on-surface font-label-caps">4 Guests</span>
          <span className="text-secondary font-semibold font-body-md">from £550 <span className="text-[12px] font-normal text-on-surface-variant">/ night</span></span>
        </div>
      </div>
      ...
      {lightbox && (
        ...
        >
          <div className="relative w-full max-w-3xl mx-4 md:mx-8" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relative aspect-[16/10] overflow-hidden bg-surface-container-low">
              ...
              <button onClick={lbPrev} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-primary-container/80 flex items-center justify-center hover:bg-primary-container transition-colors">
                <span className="material-symbols-outlined text-secondary">chevron_left</span>
              </button>
              <button onClick={lbNext} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-primary-container/80 flex items-center justify-center hover:bg-primary-container transition-colors">
            ...
            <p className="mt-3 font-label-caps text-[10px] text-on-surface-variant">OPV Crescent 2-Bed — Manchester &nbsp;·&nbsp; Use arrow keys to navigate</p>
          </div>
        </div>
      )}
    </>
